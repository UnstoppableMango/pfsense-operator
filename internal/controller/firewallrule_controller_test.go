...
var _ = Describe("FirewallRule Controller", func() {
	Context("When reconciling a resource", func() {
		...
		firewallrule := &pfsensev1alpha1.FirewallRule{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind FirewallRule")
			err := k8sClient.Get(ctx, typeNamespacedName, firewallrule)
			if err != nil && errors.IsNotFound(err) {
				resource := &pfsensev1alpha1.FirewallRule{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: pfsensev1alpha1.FirewallRuleSpec{
						Rule: pfsensev1alpha1.RuleModel{
							Ackqueue:         ptr.To(""),
							AssociatedRuleId: ptr.To(""),
							CreatedBy:        ptr.To(""),
						},
					},
				}
